...
def evaluate(model, dataloader, tokenizer, device, amp=True, recall_k_list=[5]):
    ...
    """
    print("----------zeroshot_retrieval Evaluating...-----------")
    ...
    return metrics, texts_emb.to('cpu'), images_emb.to('cpu'),texts_image_index
...
def get_text_embeddings(model, tokenizer, texts, device):
    ...
    # transfomers clip; for PLIP
        ...
        print("Loading CLIPTokenizerFast")
        inputs = tokenizer(
            texts,
            padding=True,
            truncation=True,
    ...
    # tokenizer for CONCH
        ...
        print("Loading PreTrainedTokenizerFast")
        ...
        print("Loding OtherTokenizerFast")
...
def get_image_embeddings(model, batch_images):
    ...
    if 'musk' in model_name.lower():
        ...
        image_features = F.normalize(model.encode_image(batch_images), dim=-1)
