...
class Featurizer(torch.nn.Module):
    ...
    def forward(self, input, ms_aug=None):
        ...
        elif 'visiontransformer' in model_name.lower():
            """
            视觉model
            prov-gigapath_prov-gigapath，
            other model
            """
            # print("---------visiontransformer---------")
            image_features = self.model(input)
            if self.model_id == 'Virchow2-hf-hub':
                class_token = image_features[:, 0]
                patch_tokens = image_features[:, 5:]
                image_features = torch.cat([class_token, patch_tokens.mean(1)], dim=-1)
...
def evaluate(model, train_dataloader, dataloader, fewshot_k, batch_size_eval, num_workers, lr, epochs,
    ...
    featurizer = Featurizer(model, model_id=model_name).cuda()
    ...
    if not os.path.exists(os.path.join(feature_dir, 'targets_train.pt')):
        ...
        for j, loader in enumerate([dataloader, train_dataloader]):
            ...
            num_cached = 0
            with torch.no_grad():
                for images, aug_image, target in tqdm(loader):
    ...
    # augmentation feature--only train_loader
    ...
    if augmentation:
        print('---------augmentation-----------')
        feature_aug_dir = os.path.join(feature_dir, augmentation_types)
        if not os.path.exists(feature_aug_dir):
            os.mkdir(feature_aug_dir)
        featurizer_aug = Featurizer(model, model_id=model_name).cuda()
        autocast = torch.cuda.amp.autocast if amp else suppress
        print(feature_aug_dir)
        if not os.path.exists(os.path.join(feature_aug_dir, 'targets_train_aug.pt')):
            # now we have to cache the features
            # print("cache")
            devices = [x for x in range(torch.cuda.device_count())]
            featurizer_aug = torch.nn.DataParallel(featurizer_aug, device_ids=devices)
            print("loaded featurizer_aug")
            ...
            num_cached = 0
            with torch.no_grad():
                for origin_images, images, target in tqdm(train_dataloader):
                    images = images.to(device)

                    with autocast():
                        feature = featurizer_aug(images, ms_aug)
                    ...
                    if (num_batches_tracked % 100) == 0:
                        ...
                        targets = torch.cat(targets)
                        print("save")
                        torch.save(features, os.path.join(feature_aug_dir, f'features{save_str}_cache_{num_cached}.pt'))
                        torch.save(targets, os.path.join(feature_aug_dir, f'targets{save_str}_cache_{num_cached}.pt'))
                        num_cached += 1
                        features = []
                        targets = []
                ...
                features = torch.load(os.path.join(feature_aug_dir, f'features{save_str}_cache_0.pt'))
                targets = torch.load(os.path.join(feature_aug_dir, f'targets{save_str}_cache_0.pt'))
                for k in range(1, num_cached):
                    next_features = torch.load(os.path.join(feature_aug_dir, f'features{save_str}_cache_{k}.pt'))
                    next_targets = torch.load(os.path.join(feature_aug_dir, f'targets{save_str}_cache_{k}.pt'))
                    features = torch.cat((features, next_features))
                    targets = torch.cat((targets, next_targets))

                for k in range(num_cached):
                    os.remove(os.path.join(feature_aug_dir, f'features{save_str}_cache_{k}.pt'))
                    os.remove(os.path.join(feature_aug_dir, f'targets{save_str}_cache_{k}.pt'))

                torch.save(features, os.path.join(feature_aug_dir, f'features{save_str}.pt'))
                torch.save(targets, os.path.join(feature_aug_dir, f'targets{save_str}.pt'))
        # augmentational feature
        features_aug = torch.load(os.path.join(feature_aug_dir, 'features_train_aug.pt'))
        targets_aug = torch.load(os.path.join(feature_aug_dir, 'targets_train_aug.pt'))
        assert len(features_aug) == length
        features_aug = features_aug[idxs]
        targets_aug = targets_aug[idxs]
    ...
    print(batch_size_eval)
    print(len(feature_dset))
    print("类别数量:",targets.max().item() + 1)
    ...
    return {"balanced_acc": mean_per_class_recall, "acc": acc}
