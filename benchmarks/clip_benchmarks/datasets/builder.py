...
from .histopathology_datasets import SkinDataset, PannukeDataset, UnitopathoDataset, \
    UnitopathoRetrievalDataset, PathMMUDataset, BooksetDataset, PubmedsetDataset, LC25000Dataset, Osteo, CRC100K, \
    SICAPv2, WSSS4LUAD, PatchCamelyon, PVQA, GCHTID, DIA, SGCA, BRACS_Rol, PA, PXA, PMA


def build_dataset(dataset_name, root="../dataset", transform=None, split="test", download=True, annotation_file=None,
                  language="en", task="zeroshot_classification", wds_cache_dir=None, custom_classname_file=None,
                  custom_template_file=None, corrupt=False, corrupt_level=None, corruption_types=None,augmentation= False,augmentation_types=None, **kwargs):
    """
    ...
    if dataset_name == "skin":
        ...
        assert split in ("train", "test"), f"Only `train` and `test` split available for {dataset_name}"
        ds = UnitopathoDataset(root=root,
                               transform=transform,
                               train=train,
                               corrupt=corrupt,
                               corrupt_level=corrupt_level,
        ...
        assert split in ("train", "test"), f"Only `train` and `test` split available for {dataset_name}"
        ds = UnitopathoRetrievalDataset(root=root,
                                        transform=transform,
                                        train=train,
                                        corrupt=corrupt,
                                        corrupt_level=corrupt_level,
    ...
    elif dataset_name == "pubmedset_retrieval":
        ds = PubmedsetDataset(
            root=root,
            transform=transform,
            corrupt=corrupt,
    ...
    elif dataset_name == "CRC100K_norm":
        ds = CRC100K(
            root=root,
            transform=transform,
            corrupt=corrupt,
    ...
    elif dataset_name == "BRACS_Rol_seven":
        ds = BRACS_Rol(
            root=root,
            transform=transform,
            corrupt=corrupt,
